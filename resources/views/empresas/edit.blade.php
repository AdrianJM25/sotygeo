<div x-show="openEdit" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 text-left">
    <div x-show="openEdit" x-transition.opacity @click="openEdit = false" class="fixed inset-0 bg-[#121314]/50 backdrop-blur-sm"></div>
    ...
    <div x-show="openEdit"
        ...
        <div class="p-4 border-b border-[#56928C]/20 flex justify-between items-center bg-[#56928C]">
            ...
            <button type="button" @click="openEdit = false" class="text-white/70 hover:text-white bg-white/10 hover:bg-white/20 rounded-lg p-1.5 transition-colors">
        ...
        <form action="{{ route('empresas.update', $empresa) }}" method="POST" class="p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nombre" value="{{ old('nombre', $empresa->nombre) }}" required
                    class="bg-white border {{ $errors->has('nombre') && old('is_edit') == $empresa->id ? 'border-[#B8503F] focus:ring-[#B8503F] focus:border-[#B8503F]' : 'border-[#56928C]/30 focus:ring-[#66C1BA] focus:border-[#66C1BA]' }} text-[#121314] text-sm rounded-xl focus:ring-2 block w-full p-2.5 transition-colors">
            ...
            <div>
                <label class="block mb-2 text-sm font-medium text-[#121314]">RFC</label>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" name="correo" value="{{ old('correo', $empresa->correo) }}"
                        class="bg-white border {{ $errors->has('correo') && old('is_edit') == $empresa->id ? 'border-[#B8503F] focus:ring-[#B8503F] focus:border-[#B8503F]' : 'border-[#56928C]/30 focus:ring-[#66C1BA] focus:border-[#66C1BA]' }} text-[#121314] text-sm rounded-xl focus:ring-2 block w-full p-2.5 transition-colors">
            ...
            <div class="flex items-center pt-2">
                ...
                <label for="is_active_{{ $empresa->id }}" class="ml-2 text-sm font-medium text-[#121314]">
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-[#56928C]/20">
                <button type="button" @click="openEdit = false" class="px-5 py-2.5 text-sm font-medium text-[#56928C] bg-white border border-[#56928C]/30 rounded-xl hover:bg-[#56928C]/5 transition-colors">
                    Cancelar
                </button>
